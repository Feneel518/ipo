...
export function IpoTimetable({ companyName, openDate, closeDate, allotmentDate, allotmentDateIsEstimated, refundDate, refundDateIsEstimated, creditDate, creditDateIsEstimated, expectedListingDate, listingDate }: IpoTimetableProps) {
  ...
  return (
    <section className="record-table-section" aria-label={`${companyName} IPO timetable`}>
      ...
      {milestones.map(({ label, date, estimated }, index) => (
        <div className="timetable-row" key={label}>
          ...
          <span>
            {dateLabel(date)}
            {estimated && (
              <abbr className="estimate-mark" title="Estimated date" aria-label="Estimated date">*</abbr>
            )}
          </span>
        </div>
      ))}
      <p className="gazette-footnote">* Estimated date based on the standard exchange timeline. Unmarked dates are exchange-reported.</p>
    </section>
  );
}
