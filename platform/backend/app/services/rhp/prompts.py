...
7. Preserve the financial-year labels and reported units used in the document.
   Use INR_MILLION when a table is labelled rupees in million.
...
14. For every numeric FOUND fact, cite evidence containing the exact returned number, its metric
    label, and enough table context to identify the financial period and unit. Never cite a nearby
    number or a row from a different year. If that focused support is unavailable, use NOT_FOUND
    or AMBIGUOUS instead of returning the number.
